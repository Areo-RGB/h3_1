...
export const CreateEvent: React.FC<CreateEventProps> = ({ onBack, onSuccess }) => {
  ...
  return (
    <div className="animate-in slide-in-from-right duration-300 flex flex-col h-full bg-white overflow-hidden">
      <ViewHeader id="create-event" title="Event erstellen" onBack={onBack} />
      <form
        onSubmit={handleSubmit}
        className="flex-1 overflow-y-auto p-4 pb-32 flex flex-col gap-8"
      >
        <FormSection id="event-basic">
        ...
        <BottomAction id="submit-event" label="Event erstellen" type="submit" loading={loading} />
      </form>
    </div>
  );
};
